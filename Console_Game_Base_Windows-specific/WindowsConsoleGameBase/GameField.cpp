...
void GameField::resize(size_t width, size_t height)
{
	m_Height = height;
	m_Width = width;
	this->m_Field =std::vector<std::vector<wchar_t>>(m_Height - 2, std::vector<wchar_t>(m_Width - 2, 0x0387));
}
...
void GameField::render(PaintDevice& paintDevice)
{
	...
	{
		Vector2 v4(0, i);
		paintDevice.set_char(v4, 0x2551);
